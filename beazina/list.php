...
<head>
    ...
    <title>Liste Beazina</title>
</head>

<body>
    <div class="container mt-3">
        <h2>Lisitry ny Beazina</h2>
        <p>Lisitry ny Beazina ao amin'ny Sampana Tily</p>
        <div class="row mb-3">
            <div class="col-md-6">
        ...
        <table class="table table-hover">
            ...
            $filterCondition = '';
            if (!empty($filter)) {
                $filterCondition = ' WHERE categories = :category';
            }

            $query = $conn->prepare('SELECT id_beazina, nom_beazina, prenom_beazina, totem, categories FROM beazina' . $filterCondition);
            if (!empty($filter)) {
            ...
            while ($user = $query->fetch()) {
                echo '<tr>';
                echo '<td>' . $user['nom_beazina'] . '</td>';
                echo '<td>' . $user['prenom_beazina'] . '</td>';
                ...
                echo '</td>';
                echo '<td>';
                echo '<a href="profile.php?id=' . $user['id_beazina'] . '" class="btn btn-info me-2"><i class="fa-solid fa-eye"></i></a>';
                echo '<a href="delete.php?id=' . $user['id_beazina'] . '" class="btn btn-danger"><i class="fa-solid fa-trash-can"></i></a>';
                echo '</td>';
                echo '</tr>';
            }
            ?>

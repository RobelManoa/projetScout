...
if ((!isset($_SESSION["nom_admin"]) || empty($_SESSION["nom_admin"])) && (!isset($_SESSION["tonia_id"]) || empty($_SESSION["tonia_nom"]))) {
    ...
    echo '<h1 class="text-center">Présence des Mpiandraikitra</h1>';
}

// Récupérer les catégories de beazina depuis la base de données
try {
    $query = $conn->query("SELECT idAdmin, prenom_admin, id_responsabilite FROM admins");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_presence'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO presence_admin (id_admin_present, categ_admin, dates_presence, presence) VALUES (:id_beazina, :categorie, :date_presence, 1)");
                        ...
                        <tbody>
                            ...
                            if (isset($_POST['categorie'])) {
                                ...
                                foreach ($beazinaData as $beazina) {
                                    if ($selectedCategory == 0 || $beazina['id_responsabilite'] == $selectedCategory) {
                                        echo '<tr>';
                                        echo '<td>' . $beazina['prenom_admin'] . '</td>';
                                        echo '<td>';
                                        switch ($beazina['id_responsabilite']) {
                                            case 1:
                                                echo 'Mavo';
                                                break;
                                            case 2:
                                        ...
                                        echo '</td>';
                                        echo '<td>';
                                        echo '<input type="checkbox" name="beazina[]" value="' . $beazina['idAdmin'] . '">';
                                        echo '</td>';
                                        echo '</tr>';
                                    }
                                }
